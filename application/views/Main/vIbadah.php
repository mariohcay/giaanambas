      ...
      foreach ($ibadahMingguIni as $data): ?>
        <div class="row">
          <div class="col-lg-4">
            ...
            <img class="img-fluid mx-3 my-3 shadow" style="width: 25rem;"
              src="<?= base_url('assets/img/thumbnail/') . $data['image'] ?>" alt="">
          </div>
          <div class="col-lg mx-3 my-3">
            ...
            <h6>
              <?= tgl_indo($data['tanggalIbadah'], true) . " - " . time_indo($data['jamIbadah']) . " WIB" ?>
            </h6>
            <?php
        ...
        <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            <?php foreach ($ibadah as $data): ?>
              <tr>
                <td><img class="img-responsive shadow" style="width: 10rem;"
                    src="<?= base_url('assets/img/thumbnail/') . $data['image'] ?>" alt=""></td>
                <td>
                  <?= $data['namaIbadah'] ?><br>"
                  <?= $data['temaIbadah'] ?>"
                </td>
                <td>
                  <?= tgl_indo($data['tanggalIbadah']) ?>
                </td>
                <td><a href="<?= base_url('Ibadah/lihatIbadah/') . $data['kodeIbadah'] ?>"
                    class="btn btn-info btn-sm p-2">LIHAT</a></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
